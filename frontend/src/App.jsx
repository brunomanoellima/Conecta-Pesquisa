...
// COMPONENTE DE LOADING GLOBAL (Heurística 1: Status do Sistema)
...
function Login() {
  ...
  const [isLoading, setIsLoading] = useState(false); // Heurística 1
  const [showPwd, setShowPwd] = useState(false); // Heurística 3 e 5
  ...
  return (
    <div className="h-screen flex justify-center items-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <FeedbackModal data={feedback} onClose={() => setFeedback(null)} />
      <form onSubmit={handle} className="bg-white p-8 md:p-10 rounded-2xl shadow-xl w-full max-w-md border border-white/50 animate-fadeIn">
        <div className="flex justify-center mb-6">
          <div className="bg-blue-600 text-white p-4 rounded-full shadow-lg"><FaUniversity className="text-3xl"/></div>
        </div>
        <h2 className="text-2xl font-extrabold text-gray-800 mb-2 text-center">Conecta Pesquisa</h2>
        <p className="text-center text-gray-500 mb-8 text-sm">Acesse sua conta para continuar</p>
        
        <div className="space-y-4 mb-6">
          <div>
            <label className="sr-only">E-mail</label>
            <input required type="email" className="border border-gray-300 w-full p-3.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" placeholder="Seu e-mail acadêmico" onChange={e=>setForm({...form, email:e.target.value})} disabled={isLoading} />
          </div>
          <div className="relative">
            <label className="sr-only">Senha</label>
            <input required type={showPwd ? "text" : "password"} className="border border-gray-300 w-full p-3.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition pr-12" placeholder="Sua senha" onChange={e=>setForm({...form, password:e.target.value})} disabled={isLoading} />
            <button type="button" tabIndex="-1" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition" onClick={() => setShowPwd(!showPwd)}>
              {showPwd ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
            </button>
          </div>
        </div>

        <button type="submit" disabled={isLoading} className="bg-blue-600 text-white w-full py-3.5 rounded-lg font-bold hover:bg-blue-700 transition flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
          {isLoading ? <><FaSpinner className="animate-spin" /> Entrando...</> : 'Entrar na Plataforma'}
        </button>
        <div className="mt-6 text-center border-t border-gray-100 pt-4">
          <span className="text-gray-500 text-sm">Novo por aqui? </span>
          <Link to="/register" className="text-sm font-bold text-blue-600 hover:underline">Criar Conta</Link>
        </div>
      </form>
    </div>
  );
...
function Register() {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <FeedbackModal data={feedback} onClose={() => setFeedback(null)} />
      <form onSubmit={handle} className="bg-white p-8 md:p-10 rounded-2xl shadow-xl w-full max-w-md border border-white/50 animate-fadeIn my-8">
        <h2 className="text-2xl font-extrabold text-gray-800 mb-2 text-center">Criar Nova Conta</h2>
        <p className="text-center text-gray-500 mb-8 text-sm">Preencha os dados para acessar os projetos</p>
        
        <div className="space-y-4 mb-8">
          <input required className="border border-gray-300 w-full p-3.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Nome Completo" onChange={e=>setForm({...form, nome:e.target.value})} disabled={isLoading} />
          <input required type="email" className="border border-gray-300 w-full p-3.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="E-mail Acadêmico" onChange={e=>setForm({...form, email:e.target.value})} disabled={isLoading} />
          
          <div className="relative">
            <input required type={showPwd ? "text" : "password"} minLength="6" className="border border-gray-300 w-full p-3.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none pr-12" placeholder="Crie uma Senha" onChange={e=>setForm({...form, password:e.target.value})} disabled={isLoading} />
            <button type="button" tabIndex="-1" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" onClick={() => setShowPwd(!showPwd)}>
              {showPwd ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
            </button>
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Eu sou um(a):</label>
            <select className="border border-gray-300 w-full p-3.5 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 outline-none" onChange={e=>setForm({...form, role:e.target.value})} disabled={isLoading}>
        ...
        <button type="submit" disabled={isLoading} className="bg-green-600 text-white w-full py-3.5 rounded-lg font-bold hover:bg-green-700 transition flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
          {isLoading ? <><FaSpinner className="animate-spin" /> Cadastrando...</> : 'Finalizar Cadastro'}
        </button>
        <div className="mt-6 text-center border-t border-gray-100 pt-4">
          <span className="text-gray-500 text-sm">Já possui uma conta? </span>
          <Link to="/" className="text-sm font-bold text-blue-600 hover:underline">Fazer Login</Link>
        </div>
      </form>
    </div>
  );
...
function Dashboard() {
  ...
  // Filtros (Heurística 7: Flexibilidade e Eficiência)
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Prevenção de erros
  ...
  const manageApp = async (id, status, isRemoval) => {
    ...
    setConfirmDialog({
      ...
      message: status === 'ACEITA' ? 'Este aluno será adicionado à sua equipe oficial do projeto.' : 'A candidatura será recusada. Esta ação não pode ser desfeita.',
      onConfirm: async () => {
        clearConfirm();
  ...
  const searchStudents = async () => {
    ...
    try {
      ...
      if (res.data.length === 0) notify('info', 'Busca sem resultados', `Não encontramos nenhum aluno chamado "${search}".`);
    } catch (err) {
  ...
  const renderList = (json) => { const list = safeParse(json); if (list.length > 0) return (<ul className="list-disc pl-5 space-y-1">{list.map((x,i)=><li key={i} className="text-sm text-gray-700">{x}</li>)}</ul>); return <p className="text-sm text-gray-400">Nenhum item informado.</p>; };
  const renderSkills = (json) => { const list = safeParse(json); if (list.length > 0) return list.map((x,i)=><span key={i} className="text-xs bg-blue-100 text-blue-800 px-3 py-1 rounded-full">{x}</span>); return <span className="text-xs text-gray-400 italic">O aluno ainda não preencheu suas habilidades.</span>; };
  ...
  // Filtragem local de projetos (Heurística 7)
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      <main className="w-full px-4 md:px-8 py-8 max-w-7xl mx-auto">
        ...
        {isLoading && tab !== 'projetos' ? ( <LoadingSpinner /> ) : (
          <>
          {/* CARROSSEL (Visível apenas para alunos na aba projetos) */}
                  ...
                  <Slider {...carouselSettings}>
                      {carouselSlides.map(slide => (
                          <div key={slide.id} className="h-[400px] md:h-[500px] relative overflow-hidden outline-none">
                              {/* Substitua os fundos por cores gradientes se não tiver as imagens para evitar tela em branco */}
              ...
              <div id="lista-projetos" className="animate-fadeIn w-full">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                      <div className="flex-1 w-full relative">
                        ...
                        <input type="text" placeholder="Buscar por título ou tipo (Pesquisa, Extensão...)" className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition" value={filterText} onChange={e => setFilterText(e.target.value)} />
                      </div>
                                    ...
                                    <div className="mt-auto space-y-2">
                                        ...
                                        {user.role === 'docente' && p.status !== 'CONCLUIDO' && (
                                            <div className="flex gap-2">
                                                <button onClick={()=>handleEditOrCreate(p)} className="flex-1 py-2 rounded-lg bg-blue-50 text-blue-700 font-bold hover:bg-blue-100 transition text-sm flex items-center justify-center gap-1" title="Editar informações do projeto"><FaEdit/> Editar</button>
                                                <button onClick={()=>handleCloseProject(p.id)} className="flex-1 py-2 rounded-lg bg-yellow-50 text-yellow-700 font-bold hover:bg-yellow-100 transition text-sm flex items-center justify-center gap-1" title="Fechar vagas e concluir projeto"><FaBan/> Encerrar</button>
                                                <button onClick={()=>handleDeleteProject(p.id)} className="py-2 px-3 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition flex items-center justify-center" title="Excluir projeto permanentemente"><FaTrash/></button>
                                            </div>
                                        )}
          ...
          {/* DEMAIS ABAS CONTINUAM COM O MESMO COMPORTAMENTO, APENAS ESTILOS REFINADOS (H8 e H10) */}
                  ...
                  {data.applications.filter(app => app.status === 'ACEITA').length === 0 ? (
                    ...
                    data.applications.filter(app => app.status === 'ACEITA').map(app => (
                      <div key={app.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                          ...
                          <div className="p-6">
                              <h4 className="font-bold text-gray-800 text-lg mb-4 flex items-center gap-2"><FaBullhorn className="text-blue-500"/> Quadro de Avisos do Projeto</h4>
          ...
          {tab === 'equipes' && user.role === 'docente' && (
              <div className="space-y-8 animate-fadeIn w-full max-w-5xl mx-auto">
                 <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-200 w-full">
                     ...
                     <p className="text-sm text-gray-500 mb-6">Busque por discentes cadastrados na plataforma para convidá-los ou ver seus perfis.</p>
                     ...
                     {data.projects.map(p => (
                         <div key={p.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden w-full">
                             ...
                             <div className="p-6 md:p-8">
                                 <h4 className="text-base font-bold text-gray-800 uppercase mb-4 flex items-center gap-2 border-b pb-2"><FaUsers className="text-blue-600"/> Membros da Equipe Oficial</h4>
                                 ...
                                 <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100">
                                     <h4 className="text-base font-bold text-blue-900 uppercase mb-4 flex items-center gap-2"><FaBullhorn className="text-blue-600"/> Mural de Avisos do Projeto</h4>
                                     <p className="text-sm text-blue-700 mb-4">Escreva um aviso importante para todos os alunos aprovados nesta equipe.</p>
                                     <div className="flex flex-col gap-3 mb-6">
                                         <textarea className="w-full border border-blue-200 rounded-xl p-4 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none resize-none h-24 shadow-sm" placeholder="Ex: Pessoal, nossa próxima reunião será terça-feira no lab 3..." value={postContent[p.id] || ''} onChange={(e) => setPostContent({ ...postContent, [p.id]: e.target.value })}/>
          ...
          {/* AS DEMAIS ABAS FORAM OMITIDAS AQUI PARA NÃO EXCEDER O LIMITE DE TEXTO, MAS SEGUEM O MESMO PADRÃO DA REFATORAÇÃO DE UI, CORES E COMPONENTES ACIMA. 
              (Você pode copiar o miolo de candidaturas e perfil do código original mantendo as classes estruturais do Dashboard!) 
          */}
          </>
        )}
      </main>
