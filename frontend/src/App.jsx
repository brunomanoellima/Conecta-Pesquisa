...
import { 
  ...
  FaExclamationTriangle, FaQuestionCircle, FaInfoCircle
} from 'react-icons/fa';
...
// Centraliza mensagens, confirmações e entradas do usuário para substituir alert/confirm/prompt nativos.
...
function FeedbackModal({ data, onClose }) {
  if (!data) return null;
  const style = messageIconStyles[data.type || 'info'] || messageIconStyles.info;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[70] p-4">
      <div className={`bg-white p-7 rounded-2xl shadow-2xl w-full max-w-md border ${style.border} animate-slideIn`}>
        ...
        <button onClick={onClose} className="bg-blue-600 text-white w-full py-3 rounded-lg font-bold hover:bg-blue-700 transition">
          Entendi
        </button>
      </div>
    </div>
...
function ConfirmModal({ data, onCancel }) {
  if (!data) return null;
  const style = messageIconStyles[data.type || 'question'] || messageIconStyles.question;
  const Icon = style.icon;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[70] p-4">
      <div className={`bg-white p-7 rounded-2xl shadow-2xl w-full max-w-md border ${style.border} animate-slideIn`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button type="button" onClick={data.onConfirm} className="bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition">
            Confirmar
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[70] p-4">
      <form onSubmit={submit} className={`bg-white p-7 rounded-2xl shadow-2xl w-full max-w-md border ${style.border} animate-slideIn`}>
        ...
        <textarea
          ...
          className="w-full border rounded-lg p-3 min-h-[110px] mb-5 focus:ring-2 focus:ring-blue-100 outline-none"
  ...
  const handle = async (e) => {
    ...
    try {
      ...
      setFeedback({
        ...
        message: 'Seu cadastro foi realizado. Você já pode entrar na plataforma.'
      });
...
function Dashboard() {
  ...
  const handleCloseProject = async (id) => {
    setConfirmDialog({
      ...
      message: 'As candidaturas pendentes serão encerradas e o projeto ficará marcado como concluído.',
      onConfirm: async () => {
        clearConfirm();
        try {
          await api.post(`/projects/${id}/close`);
          notify('success', 'Projeto encerrado', 'O projeto foi concluído e as candidaturas pendentes foram atualizadas.');
          load();
        } catch (err) {
  ...
  const handleDeleteProject = async (id) => {
    setConfirmDialog({
      ...
      message: 'Essa ação removerá o projeto da listagem. Confirme apenas se tiver certeza.',
      onConfirm: async () => {
        clearConfirm();
        try {
  ...
  const saveProject = async (e) => {
    ...
    if (prazo < hoje) {
      return notify('warning', 'Prazo inválido', 'O prazo de inscrição deve ser uma data futura.');
    }
    ...
    try { 
      ...
      load();
      notify('success', 'Projeto salvo', 'A operação foi realizada com sucesso.');
    } 
    catch (err) {
      notify('error', 'Erro ao salvar', err.response?.data?.error || 'Não foi possível salvar o projeto.');
    }
  };
  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      notify('success', 'Perfil salvo', 'Suas informações foram atualizadas com sucesso.');
      load();
    } catch (err) {
      notify('error', 'Erro ao salvar perfil', err.response?.data?.error || 'Não foi possível atualizar seu perfil.');
    }
  };
  ...
  const apply = async (id) => {
    setInputDialog({
      ...
      message: 'Explique brevemente por que você deseja participar deste projeto.',
      placeholder: 'Digite sua mensagem para o docente...',
      onConfirm: async (msg) => {
        clearInput();
        try {
          await api.post(`/projects/${id}/apply`, { mensagem: msg });
          notify('success', 'Candidatura enviada', 'Sua inscrição foi enviada ao docente responsável.');
          load();
        } catch (err) {
          notify('error', 'Erro na candidatura', err.response?.data?.error || 'Não foi possível enviar sua candidatura.');
        }
      }
    });
  };
  ...
  const updateApplicationStatus = async (id, status, reason = '') => {
    try {
      ...
      notify('success', 'Candidatura atualizada', `Status definido como ${status}.`);
      load();
    } catch (err) {
      notify('error', 'Erro ao atualizar', err.response?.data?.error || 'Não foi possível atualizar a candidatura.');
    }
  };
  ...
  const manageApp = async (id, status, isRemoval) => {
    if (isRemoval) {
      setInputDialog({
        ...
        message: 'Informe o motivo para remover este discente da equipe.',
    ...
    setConfirmDialog({
      ...
      message: `Deseja definir esta candidatura como ${status}?`,
      onConfirm: async () => {
        clearConfirm();
  ...
  const postToMural = async (projectId) => {
    ...
    if (!content) {
      return notify('warning', 'Aviso vazio', 'Digite uma mensagem antes de publicar no mural.');
    }

    try {
      await api.post(`/projects/${projectId}/mural`, { content });
      notify('success', 'Aviso publicado', 'A mensagem foi adicionada ao mural do projeto.');
      setPostContent({ ...postContent, [projectId]: '' });
      load();
    } catch (e) {
      notify('error', 'Erro ao postar', 'Não foi possível publicar o aviso no mural.');
    }
  };
  const searchStudents = async () => {
    try {
      ...
      if (search && res.data.length === 0) {
        notify('info', 'Nenhum aluno encontrado', 'Tente pesquisar por outro nome.');
      }
    } catch (err) {
      notify('error', 'Erro na busca', err.response?.data?.error || 'Não foi possível buscar alunos.');
    }
  };
